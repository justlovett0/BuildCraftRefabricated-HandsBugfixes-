...
public class TriggerRedstoneInput extends BCStatement implements ITriggerInternal {
   ...
   public boolean isTriggerActive(IStatementContainer container, IStatementParameter[] parameters) {
      if (container instanceof IRedstoneStatementContainer redstone) {
         int level = redstone.getRedstoneInput(null);
         if (parameters.length > 0
            && parameters[0] instanceof StatementParamGateSideOnly
            && ((StatementParamGateSideOnly)parameters[0]).isSpecific
            && container instanceof ISidedStatementContainer) {
            level = redstone.getRedstoneInput(((ISidedStatementContainer)container).getSide());
         }
   ...
   public <T> T convertTo(Class<T> clazz) {
      ...
      if (obj != null) {
         ...
         return this.active && clazz.isInstance(BCCoreStatements.ACTION_REDSTONE) ? clazz.cast(BCCoreStatements.ACTION_REDSTONE) : null;
      }
   }
}
