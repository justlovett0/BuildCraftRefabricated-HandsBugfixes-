...
public class JsonModel {
   ...
   public static JsonModel deserialize(Identifier from, ResourceLoaderContext ctx) throws JsonParseException, IOException {
      try (InputStreamReader isr = ctx.startLoading(from)) {
         return new JsonModel((JsonObject)new Gson().fromJson(isr, JsonObject.class), ctx);
      } finally {
         ctx.finishLoading();
      }
   }
   ...
   public JsonModel(JsonObject obj, ResourceLoaderContext ctx) throws JsonParseException, IOException {
      this.ambientOcclusion = GsonHelper.getAsBoolean(obj, "ambientocclusion", false);
      this.textures = JsonUtil.getSubAsImmutableMap(obj, "textures", new TypeToken<HashMap<String, String>>() {});
      if (obj.has("elements")) {
         this.cutoutElements = deserializePartArray(obj, "elements", false, ctx);
         this.translucentElements = new JsonModelPart[0];
      } else {
         this.cutoutElements = deserializePartArray(obj, "cutout", false, ctx);
         this.translucentElements = deserializePartArray(obj, "translucent", true, ctx);
      }
   }
}
